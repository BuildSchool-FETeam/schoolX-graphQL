...
import { IMiniServerService, ITestCaseConfig, TestResponse, MiniServerDTO } from './base/miniServer.interface';
...
export class PythonMiniServerService 
  ...
  async runCodeWithTestCase (code: string, testCase: ITestCaseConfig): Promise<TestResponse> {
    const result = await this.axiosInstance.post<MiniServerDTO>(
      '/python/test', 
