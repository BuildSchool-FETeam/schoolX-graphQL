...
describe('AuthResolver', () => {
  ...
  describe('signUp', () => {
    it('It should return token, userName, role', async () => {
      jest.spyOn(adminUserServiceMock, 'createUserBySignup').mockResolvedValue(
        createClientUserEntityMock({
          name: 'userName',
  ...
  describe('signIn', () => {
    it("It should throw Not found Exception if user doesn't exist", async () => {
      ...
      assertThrowError(
        resolver.signIn.bind(resolver, data),
        new NotFoundException('User is not existed')
      )
    })

    it('It should throw Exception if password is invalid', async () => {
      jest.spyOn(adminUserServiceMock, 'findUserByEmail').mockResolvedValue(
        createClientUserEntityMock({
          name: 'userName',
          password: 'password 1',
        })
